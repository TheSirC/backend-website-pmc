...
export MSG_PSQL="Suppression de la base de données"
export MSG_PSQL_SUCCESS="Suppression de la base de données réussie"
export MSG_CONFIG="Suppression des dossiers contenant la configuration de la base de données"
export MSG_CONFIG_SUCCESS="Suppression des dossiers de configuration réussie"

####################################################################
# Creating alias for `diesel_cli`
####################################################################

export DATABASE_URL="postgresql://sirc@localhost:5432/stock"
alias diesel=~/.cargo/bin/diesel

####################################################################
# Create a directory for the generated artifacts
####################################################################
...
trap \
    ...
    if  [[ -e up.sql ]]
    then
    # Saving a backup of the dump
    mv up.sql up.sql.bk
    fi

    # Dumping the content of the database
    pg_dump --host=localhost \
            --port=5432 \
            --format=p \
            --encoding=UTF-8 \
            --inserts -n "public" stock > up.sql

    # If the dump did not failed
    if [[ -s up.sql ]] 
    then 
    # Removing the backup of the dump
    rm up.sql.bk
    else
    # Restoring the backup from the dump
    mv up.sql.bk up.sql
    fi

    # Terminate the processes linked to Postgraphile
    pkill -u sirc -s 0 np?

    echo $MSG_PSQL
    pg_ctl -D $PGDATA stop && echo $MSG_PSQL_SUCCESS
    ...
    cd $PWD
    echo $MSG_CONFIG
    rm -rf $NIX_SHELL_DIR && echo $MSG_CONFIG_SUCCESS
    " \
    EXIT
...
then
    ...
    pg_ctl initdb -D $PGDATA > logs/start-db.log
...
# Recreating the working database
createdb -h $PGDATA stock

if  [[ -e up.sql ]]
then
    # Importing the dump
    psql -h $PGDATA -d stock -f up.sql > logs/init-db.log
else 
    # Importing the draft shema
    psql -h $PGDATA -d stock -f db/draft.sql
fi
